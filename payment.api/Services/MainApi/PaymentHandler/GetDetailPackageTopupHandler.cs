...
{
    public class GetDetailPackageTopupHandler : IRequestHandler<GetDetailPackageTopupRequest, IApiResponse>
    {
        private readonly PaymentPackageTelcoDbContext _dbContext;
        public GetDetailPackageTopupHandler(PaymentPackageTelcoDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<IApiResponse> Handle(GetDetailPackageTopupRequest request, CancellationToken cancellationToken)
        {
            var _package = await _dbContext.PackageTelcos.FirstOrDefaultAsync(t => t.PackageName.ToLower() == request.package_name.ToLower() && (t.PackageType == null || t.PackageType == (int)PackageType.topupdata));
